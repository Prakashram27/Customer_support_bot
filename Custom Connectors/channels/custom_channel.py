...
class RestInput(InputChannel):
    ...
    async def on_message_wrapper(
        ...
        message = UserMessage(
            text, collector, sender_id, input_channel=RestInput.name(),metadata=metadata
        )
    ...
    def _extract_metadata(self, req):
        print("User Other details ::- ",req.json.get("metadata", None))
    ...
    def stream_response(
        ...
        async def stream(resp: Any) -> None:
            ...
            task = asyncio.ensure_future(
                self.on_message_wrapper(on_new_message, text, q, sender_id,metadata)
            )
            while True:
        ...
        async def receive(request: Request):
            ...
            if should_use_stream:
                return response.stream(
                    self.stream_response(on_new_message, text, sender_id,metadata),
                     content_type="text/event-stream",
                    
                )
            else:
                collector = CollectingOutputChannel()
                on_new_message(UserMessage(text, collector, sender_id,metadata))
